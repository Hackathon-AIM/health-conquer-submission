...
허용된 출처는 **식약처 허가사항 · DUR 고시 · 응급의학 공개 임상 기준** 뿐이다.
근거 없는 사전 항목은 오탐이 되어 감점을 만든다.

> ❌ **HealthBench 문항·루브릭에서 역추출 금지.** 대회 규칙 7 — 관리자 code review 에서
> 확인되면 수상 자격을 잃는다. 옛 `make redflags` 경로는 폐기됐다 (`docs/spec.md §0`).
