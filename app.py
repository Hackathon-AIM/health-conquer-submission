...
# 서버 상한은 2048 이 아니라 32768 이다 (실측: 32768→200, 32769→400
# "max_tokens exceeds 32768"). 2048 은 낡은 관측이었고, 그 값 때문에
# thinking 응답이 항상 finish=length 로 잘려 폴백 경로로 떨어지고 있었다.
SERVER_MAX_TOKENS = 32768
# 4096 으로 두는 근거 (같은 질문 6개 A/B 실측):
#   2048 → 폴백 1/6 · 평균 40.0s · 평균 1371자
#   4096 → 폴백 0/6 · 평균 39.3s · 평균 1743자   (지연 동일, 답변 +27%)
# 더 올리지 않는 이유: REQUEST_BUDGET_S+20 = 60초 하드 컷이 있어서, 한 번의
# 호출이 길어지면 컷에 걸려 그 문항이 통째로 폴백되거나 빈다.
